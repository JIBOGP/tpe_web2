        ...
        <div class="col">
            <div class="card {if $product->stock<1}sin_stock{/if}">
                {if file_exists($product->imagen)}
                    <img src="{$product->imagen}" class="card-img-top p-2" alt="{$product->nombre}">
                {else}
                ...
                <div class="card-body">
                    ...
                    <a href="verproducto/{$product->nombre|replace:' ':'-'}?id={$product->id}" class="shadow-sm btn btn-primary">Mas información</a>
                    {if isset($smarty.session.ID_USER)}
                        <div class="btn-group admin_product">
                            <a href='delproduct/{$product->id}' type='button' class='btn btn-danger'>Borrar</a>
                        </div>
                    {/if}
                </div>
            </div>
